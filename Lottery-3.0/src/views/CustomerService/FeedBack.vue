    ...
    <div class="feedback-content">
      <div class="feedback-content-wrapper">
        <div class="selection-panel">
          <div class="title">{{ $t('quotType') }}</div>
          <div class="selection">
        ...
        <div class="message-panel">
          <div class="title">
            {{ $t('probDesc') }}
            <span>（{{ $t('theContentBetween') }}）</span>
          </div>
          <div class="message-input">
            <van-field
              v-model="message"
              type="textarea"
              :placeholder="$t('describeProb')"
              rows="10"
              minlength="20"
              maxlength="200"
              autosize />
        ...
        <div class="upload-panel">
          ...
          <div class="note">{{ $t('fileFormatDesc') }}！</div>
        </div>
        <div class="btn-panel">
          <van-button type="default" size="large">{{ $t('submit') }}</van-button>
        </div>
      </div>
      <QuestionTypesPopup
        :isShow="showPopup"
...
export default class FeedBack extends Vue {
  private pageTitle?: any = this.$t('feedback');
  private message?: string = '';
  private fileList?: any[] | any = [];
  private showPopup?: boolean = false;
  private currentQuestionType?: any = this.$t('plsSelectQuestionType');
