<template>
  <div class="pince-record">
    <Header :msg="$t('sponsoredLinks')" :leftText="$t('back')" :back="true" :rightText="$t('newPromotion')" :goToURL="'GestionPromotion'" />
          ...
          <van-list
            ...
            @load="onLoad">
              <div class="record-list-item" v-for="(item, key) in pinceRecordData" :key="key">
                ...
                <van-cell-group>
                   <van-cell :title="$t('userType') + ': '" :value="item.title" >
                     <template #right-icon>
                       <div class="row5"></div>
                    </template>
                  </van-cell>
                  ...
                  "	style="margin-top:5px;"/></center>
                   <van-cell :title="$t('promotionCode') + ': '" :value="item.number" >
                     <template #right-icon>
                       <div class="row4" v-on:click="copy(item.number)">{{ $t('copy') }}</div>
                    </template>
                  </van-cell>
                   <van-cell :title="$t('rebateRate') + ': '" :value="item.percentage" >
                     <template #right-icon>
                       <div class="row5"></div>
                    </template>
                  </van-cell>
                   <van-cell :title="$t('sponsoredLinks') + ': '" :value="item.url" >
                     <template #right-icon>
                       <div class="row4" v-on:click="copy(item.url)">{{ $t('copy') }}</div>
                    </template>
                  </van-cell>
                  <van-cell :title="$t('encryptLink') + ': '" :value="item.path" >
                     <template #right-icon>
                       <div class="row4" v-on:click="copy(item.path)">{{ $t('copy') }}</div>
                    </template>
                  </van-cell>
                </van-cell-group>
                     <div style="text-align: center; padding:30px; color:red;font-weight: bold;">{{ $t('seeDetails') }}</div>
