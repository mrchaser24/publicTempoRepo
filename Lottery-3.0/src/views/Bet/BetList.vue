<template>
  <div class="bet-list">
    <Header :leftText="'返回'" :msg="$t('lotteryList') + '('+ $t(`${gameBet.playGroup.name}`) +')'" :back="true" :style="getThemeColor.headerBackground" />
        ...
        <div class="content-top">
          <div class="top-wrapper">
            <div>
              ...
              <span style="color: #000">{{ $t('anotherBet') }}</span>
            </div>
            <div>
              <van-icon name="plus" :color="getThemeColor.plusIconColor"/>
              <span style="color: #000">{{ $t('machineSelected') }}</span>
            </div>
          </div>
        </div>
    ...
    <div class="bet-list-footer">
      <div class="bet-list-footer-wrapper">
        <div class="info-row">
          <div class="row reset">
            <van-button size="large" class="reset-button">{{ $t('reset') }}</van-button>
          </div>
          <div class="row number-input">
            <van-field v-model="betAmount" type="number" label="" :placeholder="$t('enterBetAmount')"/>
          </div>
          <div class="row bet">
            <van-button size="large" class="betting-button" :color="getThemeColor.buttonColor" @click="bet()">{{ $t('betting') }}</van-button>
          </div>  
        </div>
      </div>
    </div>
