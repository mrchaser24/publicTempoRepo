    ...
    <div class="withdraw-content">
      ...
      */ -->
      <van-cell
          center
          title
          :label="`${$t('accountBalance')}: ${user.money ? user.money : '0.00'}`"
          icon
          class="profile"
          is-link
          to=""
      ...
      <div class="input-fields">
        <van-cell-group>
          <van-field
              ...
              readonly
              clearable
              :label="$t('withdrawType') + ':'"
              :placeholder="$t('withdrawalType')"
              label-width='7.5rem'
          >
              <!-- class="w-type" -->
...
<!-- /*
  ...
  */ -->
          ...
          <van-field
              ...
              clearable
              type="password"
              :label="$t('withdrawPass') + ':'"
              :placeholder="$t('enterWithdrawPass')"
              class="w-password"
              label-width='7.5rem'
          />
        </van-cell-group>
...
export default class Withdraw extends Vue {
  ...
  */
    ...
    if (this.user.money >= this.summ && this.user.money != 0) {
      if (this.summ <= this.user.classificationModel.limitWithdrewMoney) {
        if(this.summ < 100||this.summ == ''){
          Toast(this.$t("aMinimum100"))
          return;
        }
        if (
  ...
  private selectType(val: any) {
    ...
    this.showType = false
    if (val === 0) {
      this.val1 = this.$t('银行卡')
    }
    if (val === 1) {
      this.val1 = this.$t('微信')
    }
    if (val === 2) {
      this.val1 = this.$t('支付宝')
    }
  }
