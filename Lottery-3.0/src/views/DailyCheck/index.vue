<template>
    <div class="dailyCheck">
        <Header :msg="$t('dailyCheckIn')" :leftText="$t('back')" :back="true" :HomePage="false" :rightText="''"/>
        <van-calendar
        :title="$t('viewCheckInRecords')"
        :poppable="false"
        :show-confirm="false"
        :style="{ height: '250px' }"
        type="range"
        ...
        <div class="infoSelectDays">
            <div class="title">
                    <p>{{ $t('signedFor1day') }}  <span>{{text}} </span>  {{ $t('day')}}！</p>

            </div>
            <div class="content">
                <van-button type="info" @click="showEnregistrement">{{ $t('checkIn') }}</van-button>
                ...
                <div class="Enregistrement">
                    ...
                    <van-grid direction="horizontal" :column-num="3" class="top_Enregistrement">
                        <van-grid-item  text>
                            <template #text>
                                <p>{{ $t('balance') }}</p>
                                <h5>1,174,917{{ $t('yuan') }}</h5>
                            </template>
                        </van-grid-item>
                        <van-grid-item  text>
                            <template #text>
                                <p>{{ $t('integral') }}</p>
                                <h5>22230{{ $t('minute') }}</h5>
                            </template>
                        </van-grid-item>
                        <van-grid-item  text>
                            <template #text>
                                <p>{{ $t('consecutiveCheckIn') }}</p>
                                <h5>1{{ $t('times') }}</h5>
                            </template>
                        </van-grid-item>
                    </van-grid>
                    <div class="Enregistrement_items">
                        <van-grid direction="horizontal" :column-num="3" class="title_Enregistrement">
                            <van-grid-item :text="$t('checkInDate')" />
                            <van-grid-item :text="$t('consecutiveCheckInDays')" />
                            <van-grid-item  :text="$t('pointsReward')" />
                        </van-grid>
