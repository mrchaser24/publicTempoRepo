<template>
  <div class="deposit">
    <Header :msg="pageTitle" :back="true" :leftText="$t('back')" :HomePage="false" :iconRight="''"/>
    <div class="page-content">
      ...
      <van-grid :border="false" :column-num="4" :clickable="false" class="bank-details-wrap">
        <van-grid-item
          ...
          <van-image width="25px" height="25px" fit="contain"
            :src="item.name ? require('@/assets/deposit/'+$t(`${item.name}-icon`)+'.png') : ''" />
        ...
        <div class="input-wrap">
          ...
          */ -->
          <van-field
            ...
            :rules="[{ required: true, message: $t('pleaseEnterDepositAmount') }]"
          />
        </div>
      ...
      <van-button
        ...
        class="deposit-btn"
        type="default">{{ $t('depositNow') }}</van-button>
    </div>
  </div>
</template>
<script lang="ts">
...
export default class Deposit extends Vue {
  ...
  private bankName?: any = this.$t('bankName');
  private amount?: any = ''
  private bankDetails?: any = [
    // { img: 'OnlineBankingToPay', label: this.$t('onlineBankingToPay') },
    // { img: 'BankCardTransfer', label: this.$t('bankCardTransfer') },
    // { img: 'Alipay', label: this.$t('alipay') },
    // { img: 'UnionPay', label: this.$t('unionPay') },
    // { img: 'WechatH5', label: this.$t('wechat') },
    // { img: 'Wechat', label: this.$t('wechatPay') },
    // { img: 'QQWalletH5', label: this.$t('hqWallet') },
    // { img: 'QuickPay', label: this.$t('quickPayment') },
    // { img: 'AlipayBarCode', label: this.$t('alipayScanCode') },
    // { img: 'AlipayH5', label: this.$t('alipayH5') },
  ]
  ...
  private columns?: any = [
    this.$t('constructionBank'), 
    this.$t('chinaMerchantsBank'), 
    this.$t('icbc'), 
    this.$t('abc'), 
    this.$t('bankOfChina')
  ]
  ...
  private onSelect(item:any) {
    ...
    */
    ...
    private deposit() {
      if (this.amount == '') {
        Toast(this.$t('pleaseEnterDepositAmount'));
      } else {
        // deposit API
      }
    }
