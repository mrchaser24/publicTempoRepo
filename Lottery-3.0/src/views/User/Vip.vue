    ...
    <div class="header_vip">
        ...
        >
            <template #left>
                ...
                <span @click="onClickLeft">{{ $t('back') }}</span>
            </template>
            <template #right>
        ...
        <van-cell center title :label="$t('vipDayJoined')" icon class="top_user" is-link to="DetailUser">
        <template #right-icon>
        ...
        <ul class="vip_list">
            <li>{{ $t('currentDepoist') }}（{{ $t('yuan') }}）： 0.00（0.00/500）</li>
            <li>{{ $t('currentDepoist') }}（{{ $t('yuan') }}）： 0.00（0.00/500）</li>
        </ul>
    </div>
    <van-tabs  animated class="tabs_carte_vip">
        <van-tab v-for="(index,key) in 5" :key="key" :title="'VIP ' + index" >
            <div class="carte_visa_vip">
                ...
                <van-row>
                    <van-col span="8">
                        500 <br> {{ $t('cumulativeDepoist') }}
                    </van-col>
                    <van-col span="8">
                        3000 <br> {{ $t('turnoverRequirement') }}
                    </van-col>
                    <van-col span="8">
                        2000 <br> {{ $t('reclassification') }}
                    </van-col>
                </van-row>
            </div>
            <div class="info_vip">
                <h2>VIP{{ index }}{{ $t('vipExclusivePrivileges') }}</h2>
                <van-grid direction="horizontal" :column-num="2">
                    <van-grid-item icon text>
                        <template #text>
                            <div class="text-info">
                                <h3>3</h3>
                                <p>{{ $t('dailyWithdrawals') }}</p>
                            </div>
                        </template>
                        <template #icon>
