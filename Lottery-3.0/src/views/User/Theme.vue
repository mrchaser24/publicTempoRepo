<template>
    <div class="main-theme">
        <Header :msg="$t('themeSkinning')" :back="true" :leftText="$t('back')"/>
        <van-radio-group v-model="selectedTheme" class="themes_block">
            <van-cell-group >
                <van-cell
                  v-for="(themItem, themeKey) in themeList"
                  :key="themeKey"
                  :title="themItem.title + `${getThemeColor == themItem.color ? '(' + $t('current') + ')' : ''}`"
                  clickable
...
export default class Theme extends Vue {
    ...
    private themeList: any = [
      { title: this.$t('classicBlue'), color: 'classic-blue' },
      { title: this.$t('classicRed'), color: 'classic-red' },
      // { title: this.$t('nobleBlackGold'), color: 'noble-black-gold' },
      { title: this.$t('simplePlatinum'), color: 'simple-platinum' },
    ];
