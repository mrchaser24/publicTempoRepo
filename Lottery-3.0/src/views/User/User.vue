<template>
  <div class="user">
    ...
    >
      ...
      <template #title>
        <div v-if="userData.name">
          <span class="meName">{{ userData ? userData.name : "" }}</span>
          ...
          /*
            ...
            * @Description: Decimal
          */ -->
          <span class="currency"><span class="changeToYellow">￥</span> {{(userData.money ? userData.money : '0.00')  }}</span>
        </div>
        <div v-else>
          <span class="not-login">{{ $t('loginRegister') }}</span>
        </div>
      </template>
      <template #icon>
        <img alt=""
    ...
    <div class="info_user-wrapper" v-if="false">
      ...
      <!-- <van-grid-item icon text is-link to="User/Vip">
        ...
        <van-grid-item icon text is-link to="Redeem">
          ...
          <template #text>
            <h5>
              {{ $t("redeemPoints") }} <br /><span class="tip"
                >EXCHANGE</span
              >
            </h5>
          </template>
    ...
    <van-cell-group class="link_user">
      ...
      <!-- <van-cell center :title="$t('shareApp')" icon @click="showPopup">
        <template #right-icon>
          <van-icon name="arrow" class="arrow" />
        </template>
        <template #icon>
      ...
      <!-- <van-cell center title icon is-link to="User/Theme">
        ...
        <template #title>
          <span>{{ $t("themeSkinning") }}</span>
          <van-tag type="danger">New</van-tag>
        </template>
      </van-cell> -->

      <!-- <van-cell center :title="$t('openTheApp')" icon>
        <template #right-icon>
          <van-icon name="arrow" class="arrow" />
        </template>
        <template #icon>
...
export default class User extends Vue {
  ...
  private list?: any = [
    ...
    {
      icon: "bet-record",
      name: this.$t("betRecord"),
      path: "BetRecord",
    },
    {
      icon: "transaction-record",
      name: this.$t("transactRecord"),
      path: "FundRecords",
    },
    // {
    //   icon: "dailycheckin",
    //   name: this.$t('myCollection'),
    //   path: "MyCollection"
    // },
    {
      icon: "recharge",
...
.user .header_blue.van-nav-bar {
  ...
  /*
    ...
    * @Return null
    * @Date: 2021-04-20
    * @Description: Modify design for text
  */
.changeToYellow{
  color: #f5c48c;
}
...
.user ::v-deep .icone_info_user .van-grid-item__content {
  ...
  /*
    ...
    * @Return null
    * @Date: 2021-04-20
    * @Description: Modify design icons
  */
