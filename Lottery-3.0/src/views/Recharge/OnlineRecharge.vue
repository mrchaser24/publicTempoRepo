<template>
    <div class="online-recharge">
        <Header :msg="$t('onlinePayment')" :back="true" :leftText="$t('back')"/>
                ...
                <div class="payment-method-group">
                  <div
                    ...
                    <van-image width="25" height="25" fit="contain"
                      :src="paymentItem.name ? require('@/assets/deposit/'+$t(`${paymentItem.name}-icon`)+'.png') : ''" />
                    <span>{{ $t(`${paymentItem.name}`) }}</span>
                  </div>
                </div>
            </div>
            <div class="payment-channel">
                ...
                <div class="payment-channel-group">
                  <div
                    ...
                    <span>{{ $t(`${channelItem.name}`) }}</span>
                  </div>
                </div>
            </div>
            <div class="recharge-amount">
                <div class="block-title">
                  <span>{{ $t('rechargeAmount') }}: {{ currAmount }}</span>
                </div>
                <div class="recharge-group">
                  <div class="amount-list">
                    <div
                      ...
                      <span>{{ amountItem + $t('yuan') }}</span>
                    </div>
                  </div>
                  <div class="red-text">
                      <p>{{ $t('tips') }}: {{ $t('onlineRechargeTips') }}</p>
                      <p>{{ $t('tips') }}: {{ $t('onlineRechargeTips2') }}</p>
                      <van-button
                        @click="recharge()"
                        :class="'theme-button-color'"
                        class="submit-btn"
                        type="danger">{{ $t('confirmSubmission') }}</van-button>
                  </div>
                </div>
            </div>
        </div>
...
export default class OnlineRecharge extends Vue {
    ...
    private paymentList: any = [
      // { name: this.$t('payByAlipay'), icon: 'Alipay'},
      // { name: this.$t('wechatPay'), icon: 'Wechat'},
      // { name: this.$t('qqPayment'), icon: 'QQWalletH5'},
      // { name: this.$t('unionPayment'), icon: 'UnionPay'},
      // { name: this.$t('quickPayment'), icon: 'QuickPay'},
      // { name: this.$t('onlineBankingToPay'), icon: 'OnlineBankingToPay'},
      // { name: this.$t('wechatPayment'), icon: 'WechatH5'},
      // { name: this.$t('barcodePayment'), icon: 'BarcodePayment'},
      // { name: this.$t('baiduWallet'), icon: 'BaiduWallet'},
    ];
    private paymentChannelList: any = [
      // { name: this.$t('wanpay'), icon: ''},
      // { name: this.$t('qualcomPay'), icon: ''},
      // { name: this.$t('firstPay'), icon: ''},
      // { name: this.$t('wufuPay'), icon: ''},
      // { name: this.$t('payJubao'), icon: ''},
      // { name: this.$t('qualcomPay'), icon: ''},
      // { name: this.$t('profitPayment'), icon: ''},
      // { name: this.$t('jifubao'), icon: ''},
      // { name: this.$t('beibeiPay'), icon: ''},
      // { name: this.$t('wufuPay'), icon: ''},
      // { name: this.$t('nineKPayment'), icon: ''},
      // { name: this.$t('nineKPayment'), icon: ''},
    ];
