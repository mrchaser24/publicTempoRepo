<template>
  ...
  <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
    <template #success>
      <span :style="{ color: themeColor[getThemeColor] }">{{ $t('refressSucess') }}</span>
    </template>
    
    <template #loosing>
      <span :style="{ color: themeColor[getThemeColor] }">{{ $t('looseRefresh') }}</span>
    </template>

    <template #loading>
      <van-loading :color="themeColor[getThemeColor]">
        <span :style="{ color: themeColor[getThemeColor] }">{{ $t('loading') }}</span>
      </van-loading>
    </template>
    ...
    <HomePopupNotification :text="announceText" />
    <Footer :activeTabs="0" />
  </div>
</template>
<script lang="ts">
...
import HomePopupNotification from "@/components/HomePopupNotifcation.vue";
...
.lottery_home::after,
.lottery_home ::v-deep.van-grid-item__content::after {
  display: none;
}
...
.home ::v-deep .icone_info_user .van-grid-item {
