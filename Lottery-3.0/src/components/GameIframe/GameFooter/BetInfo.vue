  ...
  <div class="bet-number-info">
    <div class="bet-info">
      <div class="bet-info-wrapper">
        <div class="info-row">
          <div class="row reset">
            <van-button size="large" class="reset-button" @click="reset">{{ $t('reset') }}</van-button>
          </div>
          <div class="row bet">
            <!-- 
            /*
            ...
            */ -->
            <van-field class="inputNumber" :class="{'en-textSize' : $i18n.locale === 'en'}" v-model="textNumber" type="number" label=""
              ...
              />
            <van-button size="large" class="betting-button" @click="bet()">{{ $t('betting') }}</van-button>
          </div>
        </div>
      </div>
    </div>
...
*/
  private bet() {
    ...
    if(parseInt(this.textNumber) > 0 && parseInt(this.textNumber) !== 0 ){
        ...
        if (this.gameBetOfficial[key].length > 0) {
          this.gameBetOfficial[key].forEach((item: any) => {
            ...
            GameService.GameBet(params).then((data) => {
              if (data.status) {
                Toast.success(this.$t('betSuccess'));
                this.reset();
                this.textNumber = '';
              } else {
                Toast.fail(this.$t(`${data.message}`));
              }
      ...
      if (betCount == 0) {
        Toast.fail(this.$t('selectBet'));
      }

    } else {
      Toast.fail(this.$t('enterAmount'))
    }
  }
