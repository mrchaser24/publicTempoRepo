<template>
  <van-popup
    ...
    <div class="pop-up-wrapper">
      ...
      <div class="pop-up-con">
        <span>{{ $t('profLoss') }}：</span>
        <span>{{ profitLoss ? profitLoss + '元' : '0.00元' }}</span>
      </div>
      <div class="pop-up-con button">
        <van-button type="default" size="small" :text="$t('recharge')" :color="'#bfc3cf'" @click="goToLink"/>
      </div>
    </div>
  </van-popup>
</template>
...
export default class GameBalance extends Vue {
  ...
  private async UserBetReport() {
    ...
    params.equal_userinfo_userBetReport = myFn.getCookie()?.userid;
