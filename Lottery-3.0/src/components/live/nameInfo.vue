<template>
  <div class="name_info">
    ...
    >
      {{
        IsAnchorFollowed(tempAnchorInfo.anchorId)
          ? $t("checkOff")
          : $t("attention")
      }}
    </van-button>
...
export default class nameInfo extends Vue {
  ...
  private async AnchorFollow() {
    ...
    params.equal_userId_anchorFollow = myFn.getCookie()?.userid;
    await AnchorFollowCoreService.Search(params).then((data) => {
      if (data.length > 0) {
        let tempAnchorList = this.filterAnchorFollow(
          data,
  ...
  private async Follow() {
    ...
    params.userId = myFn.getCookie()?.userid;
